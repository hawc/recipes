...
export default function Receipt({ post }) {
  ...
  function share(event) {
    ...
    navigator.share({text: "event.currentTarget.innerHTML"})
      .then(() => {
    ...
    renderNode: {
      [INLINES.EMBEDDED_ENTRY]: (node, children) => {
        ...
        if (node.data.target.sys.contentType.sys.id === "ingredient") {
          return (
            <span>{node.data.target.fields.amount * servings} {node.data.target.fields.name}</span>
          );
        }
      },
  ...
  return (
    <div>
      ...
      <h3>Zutaten</h3>
      <ul className={styles.ingredients} onClick={share}>{ingredients}</ul>
      <h3>Zubereitung</h3>
      <div>{documentToReactComponents(postdata.fields.description, renderOptions)}</div>
    </div>
  );
}
